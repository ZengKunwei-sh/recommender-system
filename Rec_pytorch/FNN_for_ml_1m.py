...
def train_and_eval(model, train_loader, test_loader, epochs, loss, optimizer):
	for epoch in range(epochs):
		...
		train(train_loader, model, loss, optimizer, device='cuda')
		train_auc = eval(model, train_loader, device='cuda')
		test_auc = eval(model, test_loader, device='cuda')
...
if __name__ == '__main__':
	...
	fnn = FactorizationSupportedNeuralNetworkModel(field_dims, embed_dim, [3, 3], 0.2)

	loss = BCELoss(reduction='sum').to('cuda')
	optimizer_fm = torch.optim.SGD(fm.parameters(), lr=0.01)
	optimizer_fnn = torch.optim.SGD(fnn.parameters(), lr=0.01)

	epochs = 3
	train_and_eval(fm, train_loader, test_loader, epochs, loss, optimizer_fm)

	fnn.init_embed_from_fm(fm)
	train_and_eval(fnn, train_loader, test_loader, epochs, loss, optimizer_fnn)